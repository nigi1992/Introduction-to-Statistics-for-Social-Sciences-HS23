...
summary(fit_ord)
```
But at last, I decided to go with the numeric "Total_Score" variable as the dependent variable. This way I could better introduce the "GDP per Capita" variable as a confounding variable for the second Regression model. 
```{r}
...
3. Preparing the data more carefully. Some countries were omitted in the data preparation process, because of different spelling or different names for the country names.
4. For the dependent variable I could also look at different democracy indexes such as Polity IV, Demokratie Barometer, V-Dem, etc.
5. The solution for the wide range of Population size values was adequate in this context, but perhaps a different kind of transformation would be more appropriate, ie the base-10 logarithm.
